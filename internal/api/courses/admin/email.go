...
func HandleEmail(request *EmailRequest) (*EmailResponse, *core.APIError) {
	...
	recipients, err := request.CourseMessageRecipients.ToMessageRecipients(request.Users)
	if err != nil {
		return nil, core.NewBadRequestError("-628", request, "Failed to parse email recipients.").Err(err)
	}
