...
func TestList(test *testing.T) {
	...
	for i, testCase := range testCases {
		response := core.SendTestAPIRequestFull(test, core.makeFullAPIPath(`courses/users/list`), nil, nil, testCase.email)
		if !response.Success {
...
func TestListEmptyCourse(test *testing.T) {
	...
	response := core.SendTestAPIRequestFull(test, core.makeFullAPIPath(`courses/users/list`), nil, nil, "server-admin")
	if !response.Success {
