...
func HandleTokensCreate(request *TokensCreateRequest) (*TokensCreateResponse, *core.APIError) {
	...
	if err != nil {
		return nil, core.NewUsertContextInternalError("-801", &request.APIRequestUserContext,
			"Failed to create random user token.").Err(err)
	}

	err = db.UpsertUser(request.ServerUser)
	if err != nil {
		return nil, core.NewUsertContextInternalError("-802", &request.APIRequestUserContext,
