...
func TestStaleLockRetentionWithLockedKey(test *testing.T) {
	...
	val.(*lockData).timestamp.Store(time.Now().Add(-2 * (staleDuration)).UnixNano())
...
func TestLockRetentionWithMidCheckActivity(test *testing.T) {
	...
	lockData.timestamp.Store(time.Now().Add(-2 * (staleDuration)).UnixNano())
	...
	lockData.timestamp.Store(time.Now().UnixNano())
