...
func TestStaleLockRetentionWithLockedKey(test *testing.T) {
	...
	val.(*lockData).timestamp.Store(time.Now().Add(-2 * (staleDuration)).Unix())
...
func TestLockRetentionWithMidCheckActivity(test *testing.T) {
	...
	lockData.timestamp.Store(time.Now().Add(-2 * (staleDuration)).Unix())
	...
	lockData.timestamp.Store(time.Now().Unix())
