...
def sanitize_and_import_path(path, chdir = None, **kwargs):
    """
    ...
    if (chdir is None):
        chdir = os.path.dirname(os.path.abspath(path))

    filename = os.path.basename(path)
    source_code = extract_code(path)

    return sanitize_and_import_code(source_code, filename, chdir = chdir, **kwargs)

def sanitize_and_import_code(source_code, filename, as_dict = False, chdir = None):
    """
