...
All API endpoints must reside in a directory that follows the same structure as the endpoint's base path from `internal/api`.
...
Finally, the handler function must contain the substring `Handle`.
