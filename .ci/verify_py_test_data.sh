...
# The python interface should have already been installed via '.ci/install_py_interface.sh'.

readonly THIS_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
readonly BASE_DIR="${THIS_DIR}/.."

readonly PORT='12345'
readonly TESTS_DIR="${BASE_DIR}/testdata"
...
function run() {
    cd "${BASE_DIR}"
    rm -f /autograder/tmp/autograder.pid /autograder/tmp/autograder.sock
